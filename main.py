    ...
    try:
        async with db_pool.acquire() as conn:
            return await conn.fetchrow(
                "SELECT * FROM users WHERE telegram_id::text = $1", str(telegram_id)
            )
    except Exception as e:
        ...
        async with db_pool.acquire() as conn:
            try:
                row = await conn.fetchrow(
                    ...
                    str(telegram_id), plain_password,
                )
                return row, None
            except Exception as insert_err:
                err_str = str(insert_err)
                logger.error(f"DB register_user insert error: {err_str}", exc_info=True)
                if "23505" in err_str or "unique" in err_str.lower():
                    return None, "email_exists"
                return None, err_str
    except Exception as e:
        logger.error(f"DB register_user error: {e}", exc_info=True)
        return None, str(e)
...
async def db_reload_admin_ids():
    """Recharge en mémoire les telegram_id des administrateurs depuis la base.
    Nécessaire car is_admin() s'appuie sur un cache mémoire (db_admin_telegram_ids)
    qui est vidé à chaque redémarrage du bot — sans ceci, un admin déjà connecté
    (telegram_id lié en base) perdait son statut admin après un redéploiement/restart
    et devait se reconnecter pour le retrouver."""
    if not db_pool:
        return
    try:
        async with db_pool.acquire() as conn:
        ...
        for r in rows:
            try:
                db_admin_telegram_ids.add(int(r["telegram_id"]))
            except (TypeError, ValueError):
                pass
        if rows:
            logger.info(f"✅ {len(rows)} administrateur(s) rechargé(s) depuis la base")
    except Exception as e:
        logger.error(f"Erreur db_reload_admin_ids: {e}", exc_info=True)


async def db_unlink_telegram_id(telegram_id: int) -> bool:
    """Déconnecte un compte : retire le lien telegram_id en base (logout)."""
    if not db_pool:
        return False
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE users SET telegram_id = NULL WHERE telegram_id::text = $1", str(telegram_id)
            )
        db_admin_telegram_ids.discard(telegram_id)
        return True
    except Exception as e:
        logger.error(f"Erreur db_unlink_telegram_id: {e}", exc_info=True)
        return False


async def db_list_connected_users():
    """Liste tous les comptes actuellement connectés (telegram_id lié), avec
    leur identifiant et leur temps d'abonnement restant, pour le panneau admin."""
    if not db_pool:
        return []
    try:
        async with db_pool.acquire() as conn:
            rows = await conn.fetch(
                """SELECT username, telegram_id, is_admin, is_premium,
        ...
        logger.error(f"Erreur db_list_connected_users: {e}", exc_info=True)
        return []
...
async def db_link_telegram_id(db_user_id: int, telegram_id: int) -> bool:
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE users SET telegram_id = NULL WHERE telegram_id::text = $1 AND id != $2",
                str(telegram_id), db_user_id,
            )
            await conn.execute(
...
Étape 2 : Appuie sur le bouton 💳 "Payer" dans le bot — tu seras redirigé vers le site de paiement.
Étape 3 : Effectue le paiement sur le site.
Étape 4 : Ton accès est activé automatiquement dans la base de données.
Étape 5 : Reviens dans le bot → Mon statut → bouton 🔗 Lien pour rejoindre ton canal.
Étape 6 : Tu recevras une notification automatique avec le lien dès que l'accès est confirmé.
...
- Pour renouveler : refaire le processus de paiement normalement (💳 → site de paiement → lien canal).
...
/payer — Lance directement le processus de paiement
...
- Le paiement est vérifié automatiquement via la base de données dès que l'utilisateur paie sur le site.
    ...
    if user.id in login_state:
        ...
        if step == "email":
            ...
            if not ok:
                ...
                return
            # Connexion OK — lier le telegram_id (enregistrement automatique en base)
            linked = await db_link_telegram_id(db_user["id"], user.id)
            if not linked:
                logger.error(
                    f"Échec de liaison telegram_id={user.id} au compte id={db_user['id']}"
                )
                await update.message.reply_text(
                    "⚠️ Connexion validée mais l'enregistrement de votre appareil a échoué.\n"
        ...
        if step == "first_name":
            ...
            if err:
                if err == "email_exists":
                    ...
                    logger.error(f"Inscription impossible pour user {user.id}: pool DB non initialisé (DATABASE_URL invalide ou base injoignable)")
                    msg = (
                        "⚠️ Le service est temporairement indisponible (connexion à la base de données).\n\n"
                        "Réessayez dans quelques minutes ou contactez un administrateur si le problème persiste."
                    )
                else:
...
async def handle_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_member.status in (ChatMember.MEMBER, ChatMember.ADMINISTRATOR):
        ...
        mode_emploi = (
            ...
            f"2️⃣ Appuyez sur 💳 **Payer** — vous serez redirigé vers le site de paiement.\n"
            f"3️⃣ Effectuez votre paiement sur le site.\n"
            f"4️⃣ Votre accès est activé automatiquement dans la base de données.\n"
            f"5️⃣ Revenez dans le bot → **Mon statut** → bouton **🔗 Lien** pour rejoindre votre canal.\n\n"
...
def build_admin_panel(data):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔓 Se déconnecter", callback_data="logout")],
    ]
    return text, InlineKeyboardMarkup(keyboard)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _db_user:
        logger.warning(
            f"Accès refusé: callback '{query.data}' de l'utilisateur telegram_id={update.effective_user.id} "
            f"(aucun compte lié trouvé au moment du clic)"
        )
        try:
            await query.edit_message_text(
                "🔒 **Accès refusé**\n\n"
    ...
    if query.data == "logout":
        await db_unlink_telegram_id(update.effective_user.id)
        await query.edit_message_text(
            "🔓 **Vous êtes déconnecté.**\n\n"
            "Vous devez vous reconnecter ou créer un compte pour continuer.",
            reply_markup=_auth_keyboard(),
            parse_mode="Markdown",
        )
        return
    ...
    if query.data == "admin_online_users":
        if not is_admin(update.effective_user.id):
            await query.edit_message_text("❌ Accès refusé.")
            return
        rows = await db_list_connected_users()
        if not rows:
            text = "🌐 **Utilisateurs connectés**\n\nAucun utilisateur connecté actuellement."
        else:
            lines = ["🌐 **Utilisateurs connectés**\n"]
            now = datetime.now(timezone.utc)
            for r in rows:
                role = "👑 Admin" if r["is_admin"] else ("⭐ Premium" if r["is_premium"] else "👤 Membre")
                exp = r["subscription_expires_at"]
                if exp:
                    remaining = exp - now
                    if remaining.total_seconds() > 0:
                        ...
                        m = rem // 60
                        temps = f"{h}h{m:02d}min restantes"
                    else:
                        temps = "⛔ expiré"
                else:
                    temps = "—"
                lines.append(f"{role} — `{r['username']}` (id: `{r['telegram_id']}`) — {temps}")
            text = "\n".join(lines)
        await query.edit_message_text(
            text, parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("← Retour", callback_data="admin_panel")]]),
        )
        return
    ...
    if query.data == "my_status":
        ...
        for cid, ch in channels.items():
            ...
            if uid_str in members:
                ...
                if time_left > 0:
                    ...
                    link_buttons.append([InlineKeyboardButton(
                        f"🔗 Lien — {ch.get('name', cid)}",
                        callback_data=f"get_link_{cid}"
                    )])
                else:
                    lines.append(
                        f"📢 **{ch.get('name', cid)}**\n"
        ...
        keyboard = link_buttons + [[InlineKeyboardButton("🏠 Menu principal", callback_data="back_main")]]
    ...
    if query.data == "pay_start":
        ...
        pay_keyboard = [
            [InlineKeyboardButton(
                "💳 Payer",
                url="https://paiement-s-curis-50u2.onrender.com",
            )],
            [InlineKeyboardButton("🏠 Menu principal", callback_data="back_main")],
        ]
        await query.edit_message_text(
            "💳 **Paiement d'abonnement**\n\n"
            "Cliquez sur **💳 Payer** pour accéder au site de paiement.\n\n"
            "✅ Une fois votre paiement effectué, votre accès est activé automatiquement "
            "dans la base de données.\n\n"
            "🔗 Revenez sur **Mon statut** → bouton **🔗 Lien** pour rejoindre votre canal.",
            reply_markup=InlineKeyboardMarkup(pay_keyboard),
            parse_mode="Markdown",
        )
        return

    # ── LIEN D'ACCÈS AU CANAL (bouton dans Mon statut) ─────────────────────────
    if query.data.startswith("get_link_"):
        cid = query.data[len("get_link_"):]
        user = update.effective_user
        uid_str = str(user.id)
        data = load_data()
        ...
        if not ch:
            await query.answer("❌ Canal introuvable.", show_alert=True)
            return

        current_time = int(datetime.now().timestamp())
        member = ch.get("members", {}).get(uid_str, {})
        expires_at = member.get("expires_at", 0)

        # Vérifier aussi la DB (si nouveau paiement non encore synchronisé)
        sub = await db_check_subscription(user.id)
        if sub:
            expires_ts = int(sub["expires_at"].timestamp())
            if expires_ts > expires_at:
                # Nouveau paiement détecté — mettre à jour l'accès
        ...
        if expires_at <= current_time:
            await query.answer(
                "❌ Votre accès a expiré ou n'est pas actif.\nRenouvelez votre abonnement.",
                show_alert=True
            )
            return

        # Générer un lien d'invitation à usage unique
        try:
            ...
            await query.edit_message_text(
                f"🔗 **Lien d'accès — {ch.get('name', cid)}**\n\n"
                f"👇 Cliquez sur ce lien pour rejoindre le canal:\n{invite_link}\n\n"
                f"📅 Votre accès expire le: **{expire_str}**\n\n"
                f"⚠️ Ce lien est à **usage unique** — il expire dès que vous cliquez dessus.\n"
                f"⚠️ Ne le partagez pas.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🏠 Menu principal", callback_data="back_main")]
                ])
            ...
            logger.error(f"Erreur génération lien invite {cid}: {e}")
            await query.answer("❌ Impossible de générer le lien. Contactez un administrateur.", show_alert=True)
        return
...
# ═══════════════════════════════════════════════════════════════
# TÂCHE DE SYNCHRONISATION DES PAIEMENTS DB → CANAUX (auto-notification)
# ═══════════════════════════════════════════════════════════════

async def check_db_subscriptions_task(application: Application):
    """
    Toutes les 60 secondes, interroge la base de données pour trouver les
    utilisateurs dont subscription_expires_at > maintenant ET dont le
    telegram_id est connu. Si leur accès dans channels_data.json est dépassé
    ou inexistant, on accorde l'accès et on envoie automatiquement le lien
    ...
    while True:
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                rows = await conn.fetch(
                    """
                    SELECT telegram_id, first_name, last_name, email,
                           subscription_expires_at, subscription_duration_minutes
                    FROM users
                    WHERE telegram_id IS NOT NULL
            ...
            for row in rows:
                tg_id = int(row["telegram_id"])  # colonne TEXT en DB → convertir en int
                uid_str = str(tg_id)
                expires_at_dt = row["subscription_expires_at"]
                ...
                for cid, ch in channels.items():
                    ...
                    # Si déjà à jour → ignorer
                    if existing.get("expires_at", 0) >= expires_ts:
                        continue

                    # Nouveau paiement ou renouvellement détecté
                    duration_seconds = max(expires_ts - current_time, 0)
                    ch.setdefault("members", {})[uid_str] = {
                        "expires_at": expires_ts,
                        "granted_at": current_time,
                    ...
                    try:
                        await application.bot.unban_chat_member(int(cid), tg_id, only_if_banned=True)
                    except Exception:
                        pass

                    # Envoyer notification + lien à usage unique
                    try:
                        invite_obj = await application.bot.create_chat_invite_link(int(cid), member_limit=1)
                        invite_link = invite_obj.invite_link
                        expire_str = expires_at_dt.strftime("%d/%m/%Y à %H:%M")
                        first = row["first_name"] or "utilisateur"
                        await application.bot.send_message(
                            tg_id,
                            f"🎉 **Accès activé — {ch.get('name', cid)}**\n\n"
                            f"Bonjour {first}, votre paiement a été confirmé!\n\n"
                            f"📅 Votre accès expire le: **{expire_str}**\n\n"
                            f"🔗 **Rejoignez le canal via ce lien:**\n{invite_link}\n\n"
                            f"⚠️ Ce lien est à **usage unique** — il expire dès que vous cliquez dessus.\n"
                            f"⚠️ Ne le partagez pas.",
                            parse_mode="Markdown",
                        )
                        logger.info(f"✅ Auto-notification paiement: user {tg_id} → canal {cid}")
                    except Exception as e:
                        logger.warning(f"Erreur auto-notification paiement user {tg_id} canal {cid}: {e}")

            if changed:
                save_data(data)

        except Exception as e:
            logger.error(f"Erreur check_db_subscriptions_task: {e}", exc_info=True)
...
async def main():
    ...
    asyncio.create_task(check_db_subscriptions_task(application))
